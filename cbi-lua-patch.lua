...
--       Bot Service Dashboard with PID/RSS/Route + Start/Stop/Restart buttons
--
-- APPLY INSTRUCTIONS (3 changes):
--
--   CHANGE 1: Extend the is_ajax detection line (~line 83)
--     Find:
--       local is_ajax = (http.getenv("REQUEST_METHOD") == "POST" or ...
--                        ... or http.formvalue("telemt_action"))
--     Add to the OR chain:
--       or http.formvalue("bot_action") or http.formvalue("get_bot_status")
--
...
--       end
--     (This is the end of the telemt_action handler, around line 119-120.)
--     INSERT the entire PART 1 block below AFTER that "end".
--
--   CHANGE 3: Replace the bot tab section (~lines 738-756)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PART 1: AJAX HANDLERS (insert AFTER line 120 — after telemt_action handler)
-- ─────────────────────────────────────────────────────────────────────────────
...
if http.formvalue("get_bot_status") == "1" then
    ...
    for p in raw_pids:gmatch("%d+") do
        -- Verify it's the shell bot, not the proxy binary
    ...
    if pid ~= "" then
        ...
        -- Read health state file written by the bot's health daemon
        local route = "unknown"
        local hf = io.open("/tmp/telemt_health_state", "r")
        if hf then
            local hdata = hf:read("*all") or ""; hf:close()
            -- Format: proc_state|socks_state|timestamp
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PART 2: REPLACE THE BOT TAB (replace lines 738 through 756)
-- ─────────────────────────────────────────────────────────────────────────────

-- Delete the existing block from "-- === TAB: TELEGRAM BOT ===" to just before
-- "-- === TAB: DIAGNOSTICS ===" and insert the following:
